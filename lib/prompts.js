import { getGitUserEmail, getGitUserName } from './gitUtils.js';
...
export const projectDescription = () => ({
  ...
  validate: (input) => (input?.trim() ? true : 'Project description is required'),
});
...
export const projectUrl = () => ({
  ...
  validate: (input) => {
    ...
    return isValid ? true : 'Enter a valid URL (must start with http:// or https://)';
  },
});
...
export const repoUrl = () => ({
  ...
  default: () => {
    ...
    if (gitRemoteUrl.startsWith('git@')) {
      ...
      return gitRemoteUrl.replace(/^git@/, 'https://').replace(':', '/').replace('.git', '');
    }
